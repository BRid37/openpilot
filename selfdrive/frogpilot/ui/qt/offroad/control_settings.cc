...
FrogPilotControlsPanel::FrogPilotControlsPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  for (const auto &[param, title, desc, icon] : controlToggles) {
    ...
    if (param == "AlwaysOnLateral") {
      ...
      controlToggle = new HpilotParamValueControl(param, title, desc, icon, 200, 409, std::map<int, QString>(), this, false, "");
    } else if (param == "DeltaUp") {
      controlToggle = new HpilotParamValueControl(param, title, desc, icon, 1, 7, std::map<int, QString>(), this, false, "");
    } else if (param == "DeltaDown") {
      controlToggle = new HpilotParamValueControl(param, title, desc, icon, 1, 15, std::map<int, QString>(), this, false, "");
    } else if (param == "DriverAllowance") {
      controlToggle = new HpilotParamValueControl(param, title, desc, icon, 50, 450, std::map<int, QString>(), this, false, "");
